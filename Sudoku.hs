...
getThreeByThree list = (takeDrop (get3x3 list []) [])
  where
    ...
    get3x3 [[],[],[],[],[],[],[],[],[]] list2 = list2
    get3x3 list1 list2 = get3x3 (map (\rows -> drop 3 rows) list1) (list2 ++ (map (\rows -> take 3 rows) list1))
...
solve_all_easy = solve_iter (["sudokus/easy0"++ (show a) ++ ".sud" | a <- [1..9]]++["sudokus/easy" ++ (show a) ++ ".sud" | a <- [10..50]])
solve_all_hard = solve_iter (["sudokus/hard0"++ (show a) ++ ".sud" | a <- [1..9]]++["sudokus/hard" ++ (show a) ++ ".sud" | a <- [10..95]])
